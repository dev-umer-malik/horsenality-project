...
<body>
	<div class="container-fluid my-5">
		...
		<h1 class=text-center>For {{$template->horsename}}</h1>
		<h6 class="card-subtitle mb-2 text-muted text-center">Based on data
			provided by {{$template->firstNameE}} {{$template->lastNameE}}</h6>
		<h6 class="text-center">{{$template->assessment_date}}</h6>
		...
		<div class="row">
			...
			<table class="table borderless">
				<tbody>
					<tr>
						<td class=""><img alt="" class="w-100"
							src="{{asset('assets/reports/imgs/Background (3).png')}}"></td>
						<td class=""><div class="bg-warning">
								<img alt="" class="w-75"
		...
		<div class="w-100 float-start my-5">
			<h3 class="w-50">Unique Characteristics and Specific Behaviors</h3>
			...
			<table>
				<tbody>
					<tr>
						...
						<td class="w-50"><i>When your horse acts differently from
								...
								They automatically adjust. Horses are living, breathing,
	...
	<table>
		<tbody>
			<tr>
				<td class="ps-4">
					<h3>About {{$template->horsename}}</h3>
					<div>{!!$template->horse_Description!!}</div>
				</td>
				<td><img class="p-5" alt=""
	...
	<div class="my-3">{!!$template->H_Introduction!!}</div>
	<div class="w-100">
		<table class="table">
			<tbody class="table borderless">
				<tr>
					<td class="w-25"><img class="p-4 w-75" alt=""
						src="{{asset('assets/reports/imgs/Background (11).png')}}">

						<div style="font-size: 12px;">
							<strong>Do</strong><br>These are good things to learn and
							remember so that when {{$template->horsename}} has trouble you
							can do exactly the right thing, right away.
						</div></td>
					<td class="w-75"><strong>Do's</strong>
						<div style="font-size: 12px;">{!!$template->h_dos!!}</div></td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="w-100">
		...
		<table class="table">
			<tbody class="table borderless">
				<tr>
					<td class="w-25"><img class="w-75" alt=""
						src="{{asset('assets/reports/imgs/Background (12).png')}}">

						<div style="font-size: 12px;">
							<strong>Don't</strong><br> These are the things to avoid because
					...
					<td class="w-75" style="font-size: 12px;"><strong>Don'ts</strong><br>{!!$template->h_donts!!}</td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="text-center my-2">
		<strong>How do you tell your horse to calm down? How can you motivate
			{{$template->pronoun['him-her-H']}}? How do you communicate
			what it is you want {{$template->pronoun['him-her-H']}} to
			do? What is your common language?</strong>
	</div>
	<div class="my-2">Most people will tell you to hold your horse close,
	...
	<div class="row w-100 my-4">
		<table class="table borderless">
			<tbody>
				<tr>
					<td class="w-50"><div>When you watch horses interacting and playing
	...
	<table class="table borderless">
		<tbody>
			<tr>
				<td class=""><div class="text-muted">
						<h3>Best Games and Patterns for {{$template->horsename}}</h3>
						<div>
							<br> @if(isset($template->patterns))
	...
	<div class="w-100 my-5">
		(Horsenality Detail Rating Graph) <br /> <img alt="" 
			src="{{$template->graph['Horsenality-Detail-Rating-Graph']['path']}}">
	</div>
		...
		<table class="table borderless">
			<tbody>
				<tr>
					...
					<td class=""><div class="w-75 fw-bolder float-end ps-5 text-muted">
	...
	<div class="mw-100 my-5 text-center">
		...
		<div>
			@forelse($template->H_Situational_Behaviors as $k=>$v)
			{!!$v->statement_text!!}<br /> @empty nothing to show @endforelse
		</div>
	</div>
	...
	<div class="row w-100 my-5 py-5">
		<div class="">
			<div>Let's look again at {{$template->horsename}}'s overall
			...
			<table class="table borderless">
				<tbody>
					<tr>
						<td class=""><img alt="" class="w-75"
							src="{{$template->graph['Horsenality-Detail-Rating-Graph']['path']}}"></td>
						<td class="bg-secondary text-white">{!!$template->H_Strengths_Bullets!!}
						</td>
					</tr>
				</tbody>
			</table>
		...
		<table class="table borderless">
			<tbody>
				<tr>
					...
					<td class=""><span class="text-muted">
							{!!$template->h_pat_strength!!} </span></td>
				</tr>
			</tbody>
		</table>
